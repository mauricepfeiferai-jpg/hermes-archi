...
DATE=$(date +%Y-%m-%d)
TIME=$(date +%H:%M)
OUT="$REPO/state/13/13-spec-agent_$DATE.json"
mkdir -p "$(dirname "$OUT")"
...
report = {
    "date": date,
    "time": time,
    "source": "13-spec-agent.sh",
    "role": "Spec Agent",
    "action": "Create PRODUCT.md/TECH.md for queued ideas",
...
event = {
    ...
    "type": "agent.13-spec-agent.ready",
    "source": "13-spec-agent.sh",
    "payload": report,
    "recipients": ["emperor", "13"]
}
ts = now.isoformat().replace("+00:00", "Z").replace(":", "-").replace(".", "-")
(bus_dir / f"{ts}_agent.13-spec-agent.ready.json").write_text(json.dumps(event, indent=2))
print(f"Spec Agent: {outfile}")
PYTHON
# Update handoff after loop
python3 "$REPO/control-plane/hermes/handoff_generator.py" <<EOF_H
13-spec-agent.sh: completed $(date +%Y-%m-%d-%H:%M)
EOF_H
