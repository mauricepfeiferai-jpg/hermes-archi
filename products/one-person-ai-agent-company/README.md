# Product: One-Person AI Agent Company

This folder contains the productized version of the AI Empire One-Person AI Agent Company operating system.

## Files

- `PRODUCT.md` — core product definition, pricing, pitch
- `landing-page/LANDING_PAGE.md` — landing page copy
- `sales-page/SALES_PAGE.md` — sales page copy + FAQ
- `pricing/PRICING.md` — popcorn pricing
- `checkout/CHECKOUT.md` — checkout flow

- `go-to-market/` — 14-day launch plan, cold outreach templates, lead magnet plan.
- `templates/` — link to actual template files (from `hermes-archi/templates/one-person-ai-agent-company/`)
- `assets/` — OG image, screenshots, demo assets

## Status

WORK IN PROGRESS — copy and structure ready. Next: build actual landing page HTML, create OG image, set up payment.
